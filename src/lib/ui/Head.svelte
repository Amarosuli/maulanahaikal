<script lang="ts">
	...
	$: if ($navigating) currentPage = $page.url.pathname;
...
{#if isMenuOpen}
	<div in:fade={{ easing: sineIn, duration: 200 }} out:fade={{ easing: sineOut, delay: 300, duration: 200 }} class="fixed inset-0 z-40 flex h-screen w-screen flex-col items-start justify-center gap-12 bg-slate-100 p-14">
		{#each menu as { url, name }, index}
			<a transition:fly={{ delay: 100 * index, x: -10 }} class="font-josefin-sans font-semibold transition-colors ease-in-out hover:text-orange-500 {currentPage == url ? 'text-orange-500 underline decoration-slate-800 underline-offset-4' : ''}" href={url}>{name}</a>
		{/each}
		<button onclick={() => (isMenuOpen = false)} class="fixed bottom-20 right-20 inline-flex gap-3 p-2 transition-transform ease-in active:scale-75"><X size="32" /></button>
	</div>
{/if}
